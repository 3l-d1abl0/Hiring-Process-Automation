...
<body>
	...
	<?php
			 ...
			 //echo "Feedback Summary: <br><br>";
				...
				if($stmt=mysqli_prepare($con,$q)){
						...
						mysqli_stmt_bind_result($stmt, $name, $age, $city, $coll, $role, $dt,$eval);
	...
	<?php
			...
			$q="SELECT quest FROM `ques` ORDER BY qid ASC";
				...
				if($result=mysqli_query($con,$q)){
					while($row=mysqli_fetch_assoc($result)){
						...
						echo "<br><span class='holder'> ".$row['quest']."</span> : <br> ";
						//" <span class='value'>  ".$qna[$i]."</span><br>";
						for($j=1;$j<=$qna[$i];$j++){
			...
			if($result=mysqli_query($con,$q)){
					...
					while($row=mysqli_fetch_assoc($result)){
						...
						echo "<br><span class='holder'> ".$row['ques']."</span> : <br> ";
						for($j=1;$j<=$qna[($i+$maxq)];$j++){
							echo "<div class='default_stars new_marks' ></div>";
						}
						echo"<br>";
	...
	<?php
			...
			if($hire!="no"){
					...
					$q = "UPDATE interview SET hire=1, next=0, reject=0, eval=1 WHERE id=(?) ";
				...
				*/
				...
				if($verdict=="nextr"){
					...
					$q = "UPDATE interview SET next=1, hire=0, reject=0, eval=1 WHERE id=(?) ";
					...
					*/
					...
					$q = "UPDATE interview SET reject=1, hire=0, next=0, eval=1 WHERE id=(?) ";
